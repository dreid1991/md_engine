...
state.atomParams.addSpecies(handle=mSiteHandle,mass=0.0001,atomicNum=0)
...
print 'distance between molecs 1 & 2:'
print positions[1] - positions[0]


for i in range(numMolecules):
    center = positions[i]
    molecule = create_TIP4P_2005(state,oxygenHandle,hydrogenHandle,mSiteHandle,center)

    ids = []
    for atomId in molecule.ids:
        ids.append(atomId)

    state.atoms[ids[-1]].mass = 0.00001
    rigid.createRigid(ids[0], ids[1], ids[2], ids[3])
 #   e3b3.createMolecule(ids[0], ids[1], ids[2], ids[3])
