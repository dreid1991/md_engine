...
void FixImproperHarmonic::compute() {
    ...
    compute_cu<<<NBLOCK(nAtoms), PERBLOCK, sizeof(ImproperHarmonicGPU) * maxForcersPerBlock>>>(nAtoms, state->gpd.xs(activeIdx), state->gpd.fs(activeIdx), state->gpd.idToIdxs.getTex(), forcersGPU.ptr, forcerIdxs.ptr, state->boundsGPU);
