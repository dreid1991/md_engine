#include "IntegratorReversibleVerlet.h"
...
IntegratorReversibleVerlet::IntegratorReversibleVerlet(State *statePtr)
    : Integrator(statePtr)
{

}

void IntegratorReversibleVerlet::run(int numTurns)
{
    basicPreRunChecks();
...
void IntegratorReversibleVerlet::preForce()
{
...
void IntegratorReversibleVerlet::postForce()
{
    uint activeIdx = state->gpd.activeIdx();
...
{
    py::class_<IntegratorReversibleVerlet,
               boost::shared_ptr<IntegratorReversibleVerlet>,
               py::bases<Integrator>,
               boost::noncopyable>
    (
        "IntegratorReversibleVerlet",
        py::init<State *>()
    )
    .def("run", &IntegratorReversibleVerlet::run)
    ;
}
