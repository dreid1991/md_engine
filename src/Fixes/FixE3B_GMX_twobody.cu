...
FixE3B_GMX_twobody::FixE3B_GMX_twobody(boost::shared_ptr<State> state_,
                  std::string handle_,
    ...
    if (style_ != "E3B3" && style_ != "E3B2") {
        std::cout << "FixE3B_GMX_twobody received the style argument: " << style << std::endl;
...
__global__ void updateMoleculeIdxsMap_twobody(int nMolecules, int4 *waterIds, 
...
__global__ void update_xs_twobody(int nMolecules, int4 *waterIds, real4 *mol_xs, 
...
void FixE3B_GMX_twobody::compute(int virialMode) {
    ...
    */
    ...
    if (computeVirials) {
        if (multiThreadPerAtom) {
            compute_E3B_force_twobody_2b<true,true><<<NBLOCKTEAM(nMolecules, NTPB, NTPA),NTPB,smem_twobody_force>>>(nMolecules,
                                    nPerRingPoly,
            ...
            compute_E3B_force_twobody_2b<true,false><<<NBLOCKTEAM(nMolecules, NTPB, NTPA),
        ...
        if (multiThreadPerAtom) {
            compute_E3B_force_twobody_2b<false,true><<<NBLOCKTEAM(nMolecules,NTPB, NTPA),NTPB,smem_twobody_force>>>(
                                    nMolecules,
                                    nPerRingPoly,
                                    waterIdxsGPU.data(),                      // atomIdxs for molecule idx
                                    gpdGlobal.xs(globalActiveIdx),            // as atom idxs 
            ...
            compute_E3B_force_twobody_2b<false,false><<<NBLOCKTEAM(nMolecules,NTPB,NTPA),NTPB,smem_twobody_force>>>(
                                    nMolecules,
                                    nPerRingPoly,
                                    waterIdxsGPU.data(),                      // atomIdxs for molecule idx
                                    gpdGlobal.xs(globalActiveIdx),            // as atom idxs 
...
void FixE3B_GMX_twobody::handleLocalData() {
    ...
    if (prepared) {
        ...
        update_xs_twobody<<<NBLOCK(nMolecules), PERBLOCK>>>(nMolecules, 
                                                waterIdsGPU.data(),           
        ...
        CUT_CHECK_ERROR("update_xs_twobody failed in handleLocalData!");
        ...
        updateMoleculeIdxsMap_twobody<<<NBLOCK(nMolecules),PERBLOCK>>>(nMolecules,
                                                               waterIdsGPU.data(),
                                                               waterIdxsGPU.data(),
                                                               gpdLocal.idToIdxs.d_data.data(),
                                                               gpdGlobal.idToIdxs.d_data.data());
        CUT_CHECK_ERROR("updateMoleculeIdxsMap_twobody failed in handleLocalData!");
...
bool FixE3B_GMX_twobody::stepInit(){
    /*
 ...
 *
 */
void FixE3B_GMX_twobody::singlePointEng(real *perParticleEng) {
    ...
    if (multiThreadPerAtom) {
        compute_E3B_energy_twobody_2b<true><<<NBLOCKTEAM(nMolecules,NTPB,NTPA),NTPB,smem_twobody_energy>>>(nMolecules,              // nMolecules in E3B potential
                                                                                                                                           nPerRingPoly,
                            waterIdxsGPU.data(),                      // atomIdxs for molecule idx
                            gpdGlobal.xs(globalActiveIdx),            // as atom idxs 
                            gridGPULocal.perAtomArray.d_data.data(),  // neighbor counts for molecule idx
        ...
        compute_E3B_energy_twobody_2b<false><<<NBLOCKTEAM(nMolecules,NTPB,NTPA),NTPB,smem_twobody_energy>>>(nMolecules,              // nMolecules in E3B potential
                                                                                                                                           nPerRingPoly,
                            waterIdxsGPU.data(),                      // atomIdxs for molecule idx
                            gpdGlobal.xs(globalActiveIdx),            // as atom idxs 
                            gridGPULocal.perAtomArray.d_data.data(),  // neighbor counts for molecule idx
...
void FixE3B_GMX_twobody::createEvaluator() {
    ...
    // other option
        ...
        mdError("Unknown style in FixE3B_GMX_twobody.\n");
    }
...
bool FixE3B_GMX_twobody::prepareForRun(){
...
void FixE3B_GMX_twobody::takeStateNThreadPerBlock(int NTPB) {
...
void FixE3B_GMX_twobody::takeStateNThreadPerAtom(int NTPA) {
...
void FixE3B_GMX_twobody::addMolecule(int id_O, int id_H1, int id_H2, int id_M) {
    ...
    if (! (ordered)) mdError("Ids in FixE3B_GMX_twobody::addMolecule must be as O, H1, H2, M");
...
void export_FixE3B_GMX_twobody() {
  py::class_<FixE3B_GMX_twobody, boost::shared_ptr<FixE3B_GMX_twobody>, py::bases<Fix> > 
	("FixE3B_GMX_twobody",
    ...
    .def("addMolecule", &FixE3B_GMX_twobody::addMolecule,
	     (py::arg("id_O"), 
          py::arg("id_H1"), 
          py::arg("id_H2"),
          py::arg("id_M")
    ...
    .def("handleLocalData", &FixE3B_GMX_twobody::handleLocalData)
    // the list of molecules
    .def_readonly("molecules", &FixE3B_GMX_twobody::waterMolecules)
    .def_readonly("nMolecules", &FixE3B_GMX_twobody::nMolecules)
    .def_readonly("gridGPU", &FixE3B_GMX_twobody::gridGPULocal)
    .def_readwrite("prepared", &FixE3B_GMX_twobody::prepared)
    ;
}
