...
FixE3B_GMX::FixE3B_GMX(boost::shared_ptr<State> state_,
                  std::string handle_,
    ...
    if (style_ != "E3B3" && style_ != "E3B2") {
        std::cout << "FixE3B_GMX received the style argument: " << style << std::endl;
...
void FixE3B_GMX::compute(int virialMode) {
...
void FixE3B_GMX::handleLocalData() {
    ...
    if (prepared) {
        ...
        forces_c2a1.memset(gridGPULocal.neighborlist.size());
        forces_b1a2.memset(gridGPULocal.neighborlist.size());
        forces_c1a2.memset(gridGPULocal.neighborlist.size());
 ...
 *
 */
void FixE3B_GMX::singlePointEng(real *perParticleEng) {
...
void FixE3B_GMX::createEvaluator() {
    ...
    // other option
        ...
        mdError("Unknown style in FixE3B_GMX.\n");
    }
...
void FixE3B_GMX::takeStateNThreadPerBlock(int NTPB) {
...
void FixE3B_GMX::takeStateNThreadPerAtom(int NTPA) {
...
void FixE3B_GMX::addMolecule(int id_O, int id_H1, int id_H2, int id_M) {
    ...
    if (! (ordered)) mdError("Ids in FixE3B_GMX::addMolecule must be as O, H1, H2, M");
...
void export_FixE3B_GMX() {
  py::class_<FixE3B_GMX, boost::shared_ptr<FixE3B_GMX>, py::bases<Fix> > 
	("FixE3B_GMX",
    ...
    .def("addMolecule", &FixE3B_GMX::addMolecule,
	     (py::arg("id_O"), 
          py::arg("id_H1"), 
          py::arg("id_H2"),
          py::arg("id_M")
    ...
    .def("handleLocalData", &FixE3B_GMX::handleLocalData)
    // the list of molecules
    .def_readonly("molecules", &FixE3B_GMX::waterMolecules)
    .def_readonly("nMolecules", &FixE3B_GMX::nMolecules)
    .def_readonly("gridGPU", &FixE3B_GMX::gridGPULocal)
    .def_readwrite("prepared", &FixE3B_GMX::prepared)
    ;
}
