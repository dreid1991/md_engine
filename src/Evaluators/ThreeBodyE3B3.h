         ...
         EVALUATOR eval)
{
    ...
    if (idx < nMolecules) {
        ...
        // copy the real4 vectors of the positions
        ...
        real3 fs_a1_sum = make_real3(0.0, 0.0, 0.0);
        real3 fs_b1_sum = make_real3(0.0, 0.0, 0.0);
        real3 fs_c1_sum = make_real3(0.0, 0.0, 0.0);
        ...
        for (int j = 0; j < (numNeighMolecules); j++) {
            ...
            real fs_b2a1_scalar = eval.threeBodyForceScalar(r_b2a1_magnitude);
            real fs_c2a1_scalar = eval.threeBodyForceScalar(r_c2a1_magnitude);
            real fs_b1a2_scalar = eval.threeBodyForceScalar(r_b1a2_magnitude);
            real fs_c1a2_scalar = eval.threeBodyForceScalar(r_c1a2_magnitude);
            ...
            for (int k = j+1; k < numNeighMolecules; k++) {
                ...
                real4 pos_a3_whole = xs[idx_a3];
                real4 pos_b3_whole = xs[idx_b3];
                real4 pos_c3_whole = xs[idx_c3];
                ...
                real3 r_b3a1 = bounds.minImage(pos_b3 - pos_a1);
                real3 r_c3a1 = bounds.minImage(pos_c3 - pos_a1);
               
                // -- distances vector for b3a2 and c3a2; a2 reference atom
                real3 r_b3a2 = bounds.minImage(pos_b3 - pos_a2);
                real3 r_c3a2 = bounds.minImage(pos_c3 - pos_a2);
                ...
                real r_b3a1_magnitude = length(r_b3a1);
                real r_c3a1_magnitude = length(r_c3a1);
                
                real r_b3a2_magnitude = length(r_b3a2);
                real r_c3a2_magnitude = length(r_c3a2);

                real r_b1a3_magnitude = length(r_b1a3);
                real r_c1a3_magnitude = length(r_c1a3);
                real r_b2a3_magnitude = length(r_b2a3);
                real r_c2a3_magnitude = length(r_c2a3);
        ...
        real4 fs_a1_whole = fs[idx_a1];
        real4 fs_b1_whole = fs[idx_b1];
        real4 fs_c1_whole = fs[idx_c1];
