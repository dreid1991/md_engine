        ...
        )
{
    ...
    if (idx < nMolecules * nThreadPerAtom) {
        ...
        for (int nthNeigh=myIdxInTeam; nthNeigh<numNeigh; nthNeigh+=nThreadPerAtom) {
            ...
            // RMAX_E3B_3 is defined as 0.52 + 0.09572 = 0.61572;
            ...
            if (r_a1a2 <= RMAX_E3B_3 and molIdx < otherMolIdx) {
                ...
                int pairPairIdx_b2a1, pairPairIdx_c2a1, pairPairIdx_b1a2, pairPairIdx_c1a2;
                if (MULTITHREADPERATOM) {
                    pairPairIdx_b2a1 = baseIdxE3B + myIdxInTeam + warpSize * (nthNeigh/nThreadPerAtom);
                    pairPairIdx_c2a1 = baseIdxE3B + myIdxInTeam + warpSize * (nthNeigh/nThreadPerAtom) +     memThisBlock;
                    pairPairIdx_b1a2 = baseIdxE3B + myIdxInTeam + warpSize * (nthNeigh/nThreadPerAtom) + 2 * memThisBlock;
                    pairPairIdx_c1a2 = baseIdxE3B + myIdxInTeam + warpSize * (nthNeigh/nThreadPerAtom) + 3 * memThisBlock;
                } else {
                    pairPairIdx_b2a1 = baseIdxE3B + warpSize * nthNeigh;
                    pairPairIdx_c2a1 = baseIdxE3B + warpSize * nthNeigh +     memThisBlock;
                    pairPairIdx_b1a2 = baseIdxE3B + warpSize * nthNeigh + 2 * memThisBlock;
                    pairPairIdx_c1a2 = baseIdxE3B + warpSize * nthNeigh + 3 * memThisBlock;
                }
                pairPairForces[pairPairIdx_b2a1] = make_real4(fs_b2a1);
                pairPairForces[pairPairIdx_c2a1] = make_real4(fs_c2a1);
                pairPairForces[pairPairIdx_b1a2] = make_real4(fs_b1a2);
                pairPairForces[pairPairIdx_c1a2] = make_real4(fs_c1a2);
        ...
        if (MULTITHREADPERATOM) {
            ...
            if (myIdxInTeam==0) {
                ...
                // --- but we don't know where they are in the kernel.
                /*
                real4  curTotal = e3bTotals[molIdx];
                curTotal += make_real4(smem_total[threadIdx.x]);
                ...
                */
                ...
                //atomicAdd(addr_es + 3, esSumSmem.w);
            }
...
__global__ void compute_E3B_force_threebody_3b
        (int nMolecules,
         int nPerRingPoly,
         int4 *atomsFromMolecules,     // atom idxs at a given mol idx
         const real4 *__restrict__ xs, // atom positions (as idx)
         ...
         real4 *pairPairForces,        // pair pair forces - arranged as molecule nlist, incremented by memPerBlock
         uint *computeThis,            // boolean flag (1, or 0) for given pair of molecules
         Virial *virials, 
         int nThreadPerAtom,
         EvaluatorE3B_GMX eval
    ...
    if (idx < nMolecules * nThreadPerAtom) {
        ...
        for (int nthNeigh=myIdxInTeam; nthNeigh<numNeigh; nthNeigh+=nThreadPerAtom) {
            ...
            if (computeThis[nlistIdx]) {
                ...
                //
                fs_b2a1_read = pairPairForces[pairPairIdx_b2a1];

                fs_b2a1 = make_real3(fs_b2a1_read);
                
                fs_c2a1_read = pairPairForces[pairPairIdx_c2a1];

                fs_c2a1 = make_real3(fs_c2a1_read);

                // des with itmp = ntmp*12 + (k+1)*3, k == 1, 2
                fs_b1a2_read = pairPairForces[pairPairIdx_b1a2];
                
                fs_b1a2 = make_real3(fs_b1a2_read);
                
                fs_c1a2_read = pairPairForces[pairPairIdx_c1a2];
...
__global__ void compute_E3B_energy_twobody_3b
        (int nMolecules,
         int nPerRingPoly,
         int4 *atomsFromMolecules,     // atom idxs at a given mol idx
         const real4 *__restrict__ xs, // atom positions (as idx)
    ...
    if (idx < nMolecules * nThreadPerAtom) {
        ...
        if (MULTITHREADPERATOM) {
            ...
            if (myIdxInTeam==0) {
                ...
                esCur += smem_total[threadIdx.x];
                e3bTotals[molIdx] = esCur;
            ...
            real4 totalCur = e3bTotals[molIdx];
            totalCur += totalThisMol;
            e3bTotals[molIdx] = totalCur;
...
template <bool MULTITHREADPERATOM>
__global__ void compute_E3B_energy_threebody_3b
        (int nMolecules,
         int nPerRingPoly,
         int4 *atomsFromMolecules,     // atom idxs at a given mol idx
         const real4 *__restrict__ xs, // atom positions (as idx)
