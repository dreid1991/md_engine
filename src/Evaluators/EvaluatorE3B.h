...
class EvaluatorE3B {
    ...
    public:
        ...
        /* Evaluator for E3B potential for water; see
        ...
        EvaluatorE3B(float rs_, float rf_, float E2_, 
        ...
        inline __device__ float threeBodyEnergy(float r1, float r2) {
            float sr1 = switching(r1);
            float sr2 = switching(r2);
            // s(r1)*s(r2)*expf(-k3*(r1+r2)); so, sum up all like terms, and apply A,B, or C prefactor after 
            // the summation is concluded.
            float expProduct = expf(-k3 * (r1 + r2) );
            return sr1*sr2*expProduct;
        }
        ...
        inline __device__ float dswitchdr(float dist) {
            ...
            if (dist <= rf) {
                if (dist > rs) {
                    ...
                    value -= (2.0 * (rf - dist) * (2.0 * dist + rf - rstimes3) );
        ...
        // device function to get s(r) * exp(-k3*r);
