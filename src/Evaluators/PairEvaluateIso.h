         ...
         CHARGE_EVAL chargeEval) 
{
    ...
    extern __shared__ real paramsAll[];
    int sqrSize = numTypes*numTypes;
    real *params_shr[N_PARAM];

    real3 *forces_shr;
    Virial *virials_shr;
    if (MULTITHREADPERATOM) {
        forces_shr = (real3 *) (paramsAll + sqrSize*N_PARAM);
    ...
    if (COMP_PAIRS) {
        ...
        copyToShared<real>(parameters, paramsAll, N_PARAM*sqrSize);
    ...
    // okay so we can assign multiple atoms per ring poly.  This manifests as multiple threads per bead
       ...
       // printf("force %f %f %f\n", forceSum.x, forceSum.y, forceSum.z);
        if (MULTITHREADPERATOM) {
            ...
            reduceByN_NOSYNC<real3>(forces_shr, nThreadPerAtom);
            if (myIdxInTeam==0) {
                real4 forceCur = fs[atomIdx]; 
         ...
         CHARGE_EVAL chargeEval) 
{
    ...
    if (COMP_PAIRS) {
        ...
        copyToShared<real>(parameters, paramsAll, N*sqrSize);
        __syncthreads();    
    }
    ...
    // okay so we can assign multiple atoms per ring poly.  This manifests as multiple threads per bead
        ...
        if (MULTITHREADPERATOM) {
            ...
            reduceByN_NOSYNC<real>(engs_shr, nThreadPerAtom);
            if (myIdxInTeam==0) {
         ...
         CHARGE_EVAL chargeEval) 
{
    ...
    if (idx < nAtoms*nThreadPerAtom) {
        ...
        uint32_t groupTagSelf = __real_as_uint(fs[atomIdx].w);
        ...
        for (int nthNeigh=myIdxInTeam; nthNeigh<numNeigh; nthNeigh+=nThreadPerAtom) {
            ...
            // Extract corresponding index for pair interaction (at same time slice)
            //uint otherIdx = otherIdxRaw & EXCL_MASK;
            uint32_t otherGroupTag = __real_as_uint(fs[otherIdx].w);
