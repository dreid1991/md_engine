#include "DataComputerEnthalpy.h"
...
const std::string computer_type_ = "enthalpy";

DataComputerEnthalpy::DataComputerEnthalpy(State *state_, py::list fixes_, std::string computeMode_, std::string groupHandleB_) : DataComputer(state_, computeMode_, false,computer_type_), groupHandleB(groupHandleB_) {
...
void DataComputerEnthalpy::computeScalar_GPU(bool transferToCPU, uint32_t groupTag) {
...
void DataComputerEnthalpy::computeScalar_CPU() {
    //int n;
...
void DataComputerEnthalpy::appendScalar(boost::python::list &vals) {
    vals.append(engScalar);
}

void DataComputerEnthalpy::prepareForRun() {
    if (fixes.size() == 0) {
